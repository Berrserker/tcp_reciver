...
{
    stream >> m_filePath;
    m_filePath = std::regex_replace(m_filePath, std::regex("_"), " ");
    std::cout << "\n+ path +" << m_filePath;
    stream >> m_fileName;
    std::cout << "\n+ name +" << m_fileName;
    stream >> m_fileSize;
...
void Session::doReadFileContent(size_t t_bytesTransferred)
{
    if (t_bytesTransferred > 0) {
        ...
        if (m_outputFile.tellp() >= static_cast<std::streamsize>(m_fileSize)) {
            ...
            boost::filesystem::path sfilePath = m_fileName;
            boost::filesystem::path efilePath = m_filePath;
            auto manage_path = efilePath.parent_path();
            
            if (exists(manage_path))
            {
                boost::filesystem::path efilePath = m_filePath;
                boost::filesystem::copy_file(sfilePath, efilePath, boost::filesystem::copy_option::overwrite_if_exists);
            }
            else {
                std::cout << "destination is broken, yr file in work dir";
            }
